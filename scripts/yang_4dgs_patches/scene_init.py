...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], num_pts=100_000, num_pts_ratio=1.0, time_duration=None):
        ...
        elif os.path.exists(os.path.join(args.source_path, "dataset.json")) and \
             os.path.exists(os.path.join(args.source_path, "camera")):
            print("Found dataset.json + camera/, assuming HyperNeRF data set (NeRFies format)!")
            scene_info = sceneLoadTypeCallbacks["HyperNeRF"](args.source_path, args.white_background, args.eval, num_pts=num_pts, time_duration=time_duration, extension=args.extension, num_extra_pts=args.num_extra_pts, frame_ratio=args.frame_ratio, dataloader=args.dataloader)
        else:
