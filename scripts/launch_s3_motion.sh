#!/usr/bin/env bash
# S³-geodesic-motion probe: R(dt) = exp(dt·ω̂) ∈ SO(3) rotates V_k and conjugates Σ_3D.
# Per-Gaussian ω ∈ R^3 init zero. NO quadratic motion (c2) in this evaluation.
...
# Baseline lr_omega=5e-4
nohup modal run --detach scripts/train_modal.py "${BASE_FIXED[@]}" \
    --use-s3-motion --lr-omega 5e-4 \
    --run-tag bugF-s3motion-lr5e4 \
    > /tmp/probe_bugF-s3motion-lr5e4.log 2>&1 &
echo "Launched s3motion lr=5e-4 — PID $!"

# Bigger lr_omega to test if ω needs more learning headroom
nohup modal run --detach scripts/train_modal.py "${BASE_FIXED[@]}" \
    --use-s3-motion --lr-omega 5e-3 \
    --run-tag bugF-s3motion-lr5e3 \
    > /tmp/probe_bugF-s3motion-lr5e3.log 2>&1 &
echo "Launched s3motion lr=5e-3 — PID $!"

echo ""
echo "Bug F reference: 24.93 dB val, 86k Gaussians, 371s wall."
echo "Compare against parallel quad-motion runs (launch_quad_motion.sh)."
